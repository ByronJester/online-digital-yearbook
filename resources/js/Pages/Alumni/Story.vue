...
const toggleLike = async(post, status) => {
    ...
    await Inertia.post(route('staff-ss-save-like'), formData, {
        headers: {
            'Content-Type': 'multipart/form-data',
        },
        onSuccess: (page) => {
...
const addComment = async(post, commentText) => {
    if(!commentId.value) {
        ...
        await formComment.post(route('staff-ss-save-comment'), {
            onSuccess: (page) => {
                location.reload()
            },
            onError: (errors) => {
        ...
        await Inertia.post(route('staff-ss-edit-comment'), formData, {
...
const deleteComment = (id) => {
    ...
    .then((proceed) => {
        if (proceed) {
            ...
            Inertia.post(route('staff-ss-delete-comment'), formData, {
                headers: {
                    'Content-Type': 'multipart/form-data',
                },
                onSuccess: (page) => {
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <!-- News feed posts -->
        <div class="space-y-6">
            <div v-for="post in stories" :key="post.id" class="border rounded-lg p-4 bg-white h-full">
                <div class="font-bold">
                ...
                <div v-if="post.file" class="my-4 flex justify-center items-center">
